...
def test_start_and_question_cycle(tmp_path):
    ...
    resp = client.post("/api/v1/interview/start", json=payload)
    assert resp.status_code == 200
    data = resp.json()
    ...
    # get question
    q1 = client.get(f"/api/v1/interview/question/{session_id}")
    ...
    s = client.post("/api/v1/interview/answer", json=ans_payload)
...
def test_evaluate_score_and_report_flow():
    ...
    q = client.get(f"/api/v1/interview/question/{session_id}")
    ...
    e = client.post("/api/v1/interview/evaluate", json=eval_payload)
    assert e.status_code == 200
    eval_result = e.json()
    assert "score" in eval_result

    # score
    sc = client.get(f"/api/v1/interview/score/{session_id}")
    ...
    r = client.get(f"/api/v1/interview/report/{session_id}")
...
def test_invalid_session_and_bad_requests():
    # missing session
    missing = client.get("/api/v1/interview/question/not-a-session")
